...
list = dir(fullfile(fp_s,'**','*_setme.csv'))

list2 = struct2table(dir(fullfile(fp_s,'**','*_setstim.txt')))

list3 = dir(fullfile(fp_s,'**','*_quest.xlxs'))

ID = strings(size(list,1),1)
CS = strings(size(list,1),1)
Stim = nan(size(create,1),1)
%DONT UNDERSTAND HOW THIS WORKS FOR NOW

for idx = 1:size(list,1)  
    
    fcur = fileread([list(idx).folder filesep list(idx).name])
