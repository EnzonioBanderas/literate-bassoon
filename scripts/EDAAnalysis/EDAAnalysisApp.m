classdef EDAAnalysisApp < handle
    %
    % Usage:
    %     app.EDAAnalysisApp
    ...
    properties (Constant)
        AppPathBase = {'/EDAAnalysis'};
        AppClass = 'EDAAnalysisApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            EDAAnalysisApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'EDAAnalysis')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong EDAAnalysis
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'EDAAnalysis').getString, ...
            ...
            if nargout(@EDAAnalysis) == 0
                eval('EDAAnalysis');
            else
                obj.AppHandle = eval('EDAAnalysis');
            end
