function [t_converted,t_excluded] = essbids_dicom2bids(fp_dicomfolder,fp_bids,additionalInfo,varargin)
%% ESSBIDS_DICOM2BIDS
%
...
%
%  Inputs: 
%    fp_dicomfolder : folder of a singe subject dicom data
...
%
%  Changelog:
%   2022-07-14 : delete temp folder if only niitable.mat left
%   2022-07-12 : add diary function
%   2022-07-08 : attributing fmaps: allow to ignore acq field only present
%                in fmap, but not in bold or dwi file. E.g.,
%                      fmap/sub-001_acq-dwi_fmap.nii.gz
%                      dwi/sub-001_dwi.nii.gz
%   2022-07-06 : add sernum to dcm2niix output filenames
%
%
  ...
  files2ignore = {'AAhead','localizer','_MPR_','_TRACE','_TENSOR_B0',...
    '_FA.','_ColFA.','_ADC.','AAHead','tfl_b1map','_Aspire_','_T2star_', ...
    'dummy', '_MIP_Collection_'};
  ...
  %% start diary if not already running
  diary_status = get(0,'diary');
  fn_diary = fullfile(fp_bids,'log_dicomimport.txt');
  if strcmpi(diary_status,'off')
    fprintf('Printing to diary file\n  %s\n',fn_diary);
    diary(fn_diary);
  end
...
%   t1.AcquisitionNumber = nan(numel(fl),1);
  ...
  t1.AcquisitionTime  = repmat({''},numel(fl),1);
  t1.SizeInMB         = nan(numel(fl),1);
  ...
  for i=1:numel(fl)
    fn_old = fl{i};
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     fn_old = strrep(fn_old,'_UNI_Images','_UNI');
%     fn_old = strrep(fn_old,'_INV1','_inv-1');
%     fn_old = strrep(fn_old,'_INV2','_inv-2');
%     fn_old = strrep(fn_old,'_UNI','_UNIT1');
%     fn_new = strrep(fn_old,'_Susceptibility', '_QSM_anat_ses-1_Susceptibility');
%     fn_new = strrep(fn_old,'_Brainmask',      '_QSM_anat_ses-1_Brainmask');
%     fn_new = strrep(fn_old,'_TissuePhase',    '_QSM_anat_ses-1_TissuePhase');
%     fn_new = strrep(fn_old,'_UnwrappedPhase', '_QSM_anat_ses-1_UnwrappedPhase');
    fn_new = fn_old;
    if ~strcmp(fn_old, fn_new)
        movefile(fn_old, fn_new);
    end
    fn_old = fn_new;
    fl{i} = fn_old;
    ...
    finfo = dir(fn_old);
    t1.SizeInMB(i) = round(finfo.bytes/2^20,2); %conversion bytes to MB

    bf1 = essbids_parseLabel(fn_old,'ignoreFields',{'sernum'});
    ...
    if isfield(bf1,'suffix')
      ...
      if strcmpi(suffix,'images') && contains(bf1.fname,'_UNI_Images')
        suffix = 'UNI';
      end
    else
      suffix = '';
...
%         end
%     end
    if strcmp(suffix,'MP2RAGE')
      tmp = regexp(bf1.fname,'INV(?<inv>[0-9]*)','names');
      if ~isempty(tmp)
        bf1.inv =tmp(end).inv;
      elseif regexp(bf1.fname,'UNI')
        bf1.suffix = 'UNIT1';
      else
        tmp = strsplit(bf1.fname,'_');
        tmp = tmp(ismember(lower(tmp),lower(types_suffix)));
...
%     end
    if isfield(t1.jinfo{i},'AcquisitionTime')
      t1.AcquisitionTime{i} = t1.jinfo{i}.AcquisitionTime(1:8);
    else
      t1.AcquisitionTime{i} = 'n/a';
    end
  end
...
%         ind1 = ind1(1);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if strcmp(t1.bidsflags{ind1(iIND1)}.type,'fmap')
          if ismember('m',lower(t1.jinfo{ind1(iIND1)}.ImageType))
            t1.bidsflags{ind1(iIND1)}.suffix = sprintf('magnitude%1.0f',j);
            t1.jinfo{ind1(iIND1)}.EchoNumber = j;
            fprintf('%0.5f : magnitude%d\n',te_s(j),j)
          elseif ismember('phase',lower(t1.jinfo{ind1(iIND1)}.ImageType))
            t1.bidsflags{ind1(iIND1)}.suffix = sprintf('phase%1.0f',j);
            t1.jinfo{ind1(iIND1)}.EchoNumber = j;
            fprintf('%0.5f : phase%d\n',te_s(j),j)
          end
          
        else
          t1.bidsflags{ind1(iIND1)}.echo = sprintf('%.0f',j);
          t1.jinfo{ind1(iIND1)}.EchoNumber = j;
          if strcmpi(t1.bidsflags{ind1(iIND1)}.suffix,'gre')
            t1.bidsflags{ind1(iIND1)}.suffix = 'MEGRE';
          end
          fprintf('%0.5f : echo-%d\n',te_s(j),j)
        end 
        end
...
%       end
    ...
    fn = fn(not(ismember(fn,...
      {'suffix','type','fname','fpath','extension','dir'})));
    fl_IntendedFor = {};
    for j=1:size(t1,1)
  ...
  if size(t_excluded,1)>0
    warning('No unique file names could be drived for:\n%s',...
...
%     txt_bi0 = fprintf('%s%s\n',fn,fe);
  ...
  try
    dl=struct2table(dir(dir_temp));
    % delete folder if empty or only niitable.mat present
    if prod(ismember(dl.name,{'.','..','niitable.mat'}))
      rmdir(dir_temp,'s');
    end
  catch
  end
  
  %% switch diary back to previous status
  diary(diary_status)
